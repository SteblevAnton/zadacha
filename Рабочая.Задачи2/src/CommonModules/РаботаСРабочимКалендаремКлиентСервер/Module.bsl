...
Процедура ПрименитьНастройкиОтображения(Планировщик, НастройкиОтображения) Экспорт
	...
	ОтображатьКоличествоДнейВНеделе = НастройкиОтображения.ОтображатьКоличествоДнейВНеделе;
	...
	Для Каждого ВыделеннаяДата Из ВыделенныеДаты Цикл
		...
		ОтображаемаяДатаОкончания = ПолучитьДатуОкончанияОтображаемогоПериода(ПериодОтображения, ВыделеннаяДата, ОтображатьКоличествоДнейВНеделе);
	...
	Если НастройкиОтображения.ОтображатьСобытия Тогда
		Если НастройкиОтображения.Пользователи.Количество() > 1 Тогда
			ИзмерениеПланировщика = Планировщик.Измерения.Добавить("Сотрудник");
...
Процедура СкорректироватьДатуНачалаИОкончания(
	...
	Если ИзменятьДатуНачалаИОкончания Тогда
		...
		Если ВесьДень Тогда
			...
			Если ДатаНачала <> НачалоДня(ДатаНачала) + 60 * 60 * 9 Тогда
				ДатаНачала = НачалоДня(ДатаНачала) + 60 * 60 * 9;
			КонецЕсли;
			
			Если ДатаОкончания <> КонецДня(ДатаОкончания) - 60 * 60 * 6 Тогда
				ДатаОкончания = КонецДня(ДатаОкончания) - 60 * 60 * 6;
			КонецЕсли;
...
Функция ПолучитьДатуОкончанияОтображаемогоПериода(ПериодОтображения, Дата, ОтображатьКоличествоДнейВНеделе = Неопределено) Экспорт
	...
	ИначеЕсли ПериодОтображения = ПредопределенноеЗначение("Перечисление.ПериодОтображенияРабочегоКалендаря.Неделя") Тогда
		Если ОтображатьКоличествоДнейВНеделе = ПредопределенноеЗначение(
			"Перечисление.ЦБР_КоличествоДнейВКалендаре.Семидневка") Тогда

			ДатаОкончанияПериода = КонецНедели(Дата);

		ИначеЕсли ОтображатьКоличествоДнейВНеделе = ПредопределенноеЗначение(
			"Перечисление.ЦБР_КоличествоДнейВКалендаре.Шестидневка") Тогда

			ДатаОкончанияПериода = КонецНедели(Дата) - 86400;

		ИначеЕсли ОтображатьКоличествоДнейВНеделе = ПредопределенноеЗначение(
			"Перечисление.ЦБР_КоличествоДнейВКалендаре.Пятидневка") Тогда
	...
	ИначеЕсли ПериодОтображения = ПредопределенноеЗначение("Перечисление.ПериодОтображенияРабочегоКалендаря.ДвеНедели") Тогда
		...
		Если ОтображатьКоличествоДнейВНеделе = ПредопределенноеЗначение(
			...
			ДатаОкончанияПериода = КонецНедели(Дата) + 604800;
		ИначеЕсли ОтображатьКоличествоДнейВНеделе = ПредопределенноеЗначение(
			"Перечисление.ЦБР_КоличествоДнейВКалендаре.Шестидневка") Тогда

			ДатаОкончанияПериода = КонецНедели(Дата) + 604800 - 86400;

		ИначеЕсли ОтображатьКоличествоДнейВНеделе = ПредопределенноеЗначение(
			"Перечисление.ЦБР_КоличествоДнейВКалендаре.Пятидневка") Тогда

			ДатаОкончанияПериода = КонецНедели(Дата) + 604800 - 2 * 86400;
		Иначе
...
Функция ПолучитьТекстовоеПредставлениеПовторения(НастройкиПовторения) Экспорт
	...
	ИначеЕсли НастройкиПовторения.ЧастотаПовторения =
		ПредопределенноеЗначение("Перечисление.ЦБР_ЧастотаПовторения.Ежедневно") Тогда
		
		ПредставлениеПовторения = НСтр("ru = 'Каждый день'");
		
	ИначеЕсли НастройкиПовторения.ЧастотаПовторения =
		ПредопределенноеЗначение("Перечисление.ЦБР_ЧастотаПовторения.Еженедельно") Тогда
		
		ПредставлениеПовторения = НСтр("ru = 'Каждую неделю'");
		
	ИначеЕсли НастройкиПовторения.ЧастотаПовторения =
		ПредопределенноеЗначение("Перечисление.ЦБР_ЧастотаПовторения.Ежемесячно") Тогда
		
		ПредставлениеПовторения = НСтр("ru = 'Каждый месяц'");
		
	ИначеЕсли НастройкиПовторения.ЧастотаПовторения =
		ПредопределенноеЗначение("Перечисление.ЦБР_ЧастотаПовторения.Ежегодно") Тогда
...
//
Функция ПолучитьИндексЦвета(Цвет, Отработана = Ложь) Экспорт
	
	ИндексЦвета = 0;
	
	Если Отработана Тогда
		ИндексЦвета = 6;
	ИначеЕсли Цвет = ПредопределенноеЗначение("Перечисление.ЦБР_ЦветаРабочегоКалендаря.Нет") Тогда
		ИндексЦвета = 0;
	ИначеЕсли Цвет = ПредопределенноеЗначение("Перечисление.ЦБР_ЦветаРабочегоКалендаря.Красный") Тогда
		ИндексЦвета = 1;
	ИначеЕсли Цвет = ПредопределенноеЗначение("Перечисление.ЦБР_ЦветаРабочегоКалендаря.Синий") Тогда
		ИндексЦвета = 2;
	ИначеЕсли Цвет = ПредопределенноеЗначение("Перечисление.ЦБР_ЦветаРабочегоКалендаря.Желтый") Тогда
		ИндексЦвета = 3;
	ИначеЕсли Цвет = ПредопределенноеЗначение("Перечисление.ЦБР_ЦветаРабочегоКалендаря.Зеленый") Тогда
		ИндексЦвета = 4;
	ИначеЕсли Цвет = ПредопределенноеЗначение("Перечисление.ЦБР_ЦветаРабочегоКалендаря.Оранжевый") Тогда
		ИндексЦвета = 5;
	КонецЕсли;
...
Функция СформироватьЭлементЗаписиКалендаря(ЗаписьКалендаря) Экспорт
	...
	Если ЗаписьКалендаря.ТипЗаписиКалендаря
		...
		ПредопределенноеЗначение("Перечисление.ЦБР_ЭлементыРабочегоКалендаря.СобытиеПовторяющееся");
	КонецЕсли;
...
Функция НовыйЭлементЗаписиКалендаря()
	...
	ПредопределенноеЗначение("Справочник.ЦБР_ЗаписиРабочегоКалендаря.ПустаяСсылка"));

...
//
Функция ПодразделенияПодчиненных(Пользователь, ТолькоДействующие = Истина) Экспорт
	
	ПодразделенияПодчиненных = Новый ТаблицаЗначений;
	ПодразделенияПодчиненных.Колонки.Добавить("Подразделение");
	ПодразделенияПодчиненных.Колонки.Добавить("ПодчиненныйПользователь");
	
	// Получим подчиненные подразделения.
	ПодчиненныеПодразделения = ПодчиненныеПодразделения(Пользователь);
	...
	Для Каждого КлючИЗначение Из СотрудникиВПодразделениях Цикл
		...
		Для Каждого Сотрудник Из СотрудникиПодразделения Цикл
			...
			СтрокаПодчиненного.ПодчиненныйПользователь = Сотрудник;
...
//
// Возвращаемое значение:
//  Массив из СправочникСсылка.Пользователи - Подчиненные пользователи.
//
Функция ПодчиненныеПользователи(Пользователь, ТолькоДействующие = Истина) Экспорт
	
	ПодразделенияПодчиненных = ПодразделенияПодчиненных(Пользователь, ТолькоДействующие);
	ПодчиненныеПользователи = ПодразделенияПодчиненных.ВыгрузитьКолонку("ПодчиненныйПользователь"); 
...
Функция СформироватьДанныеВыбора(Текст, Знач ДополнениеТипа = Неопределено, ТипОбъекта = Неопределено) Экспорт
	...
	Если ДополнениеТипа.СодержитТип(Тип("СправочникСсылка.АвтоподстановкиДляОбъектов")) Тогда
		...
		Запрос.УстановитьПараметр("Автоподстановка", НСтр("ru = 'автоподстановка'"));
...
Функция ЭтоРуководитель(Пользователь) Экспорт 
	...
	Запрос = Новый Запрос(
		...
		|	И СтруктураПредприятия.Руководитель В (&ПользовательИСотрудники)");
	
	ПользовательИСотрудники = Сотрудники.ПользовательИСотрудники(Пользователь); 		
	Запрос.Параметры.Вставить("ПользовательИСотрудники", ПользовательИСотрудники);
...
//
Функция ЭтоОтветственныйЗаНСИ() Экспорт
	
	ЭтоОтветственныйЗаНСИ = Пользователи.ЭтоПолноправныйПользователь()
		Или РольДоступна("ДобавлениеИзменениеНСИ");

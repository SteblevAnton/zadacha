...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Сотрудник) Тогда
		Сотрудник = Пользователи.АвторизованныйПользователь();
	КонецЕсли;
	
	УстановитьПривилегированныйРежим(Истина);
	НастройкаДоступногоВремени = РегистрыСведений.ДоступноеВремяСотрудников.СоздатьНаборЗаписей();
	НастройкаДоступногоВремени.Отбор.Сотрудник.Установить(Сотрудник);
	НастройкаДоступногоВремени.Прочитать();
	УстановитьПривилегированныйРежим(Ложь);
	
	ДоступностьСотрудника = РаботаСРабочимКалендаремСервер.ПолучитьНастройкуДоступностиСотрудника(
		Сотрудник);
	
	Если ДоступностьСотрудника <> Перечисления.ЦБР_ДоступноеВремяСотрудников.ДоступенПоРасписанию 
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьГрафикиРаботы") Тогда
			...
			Для Каждого ЭлементРабочееВремя Из ГрафикРаботы.РабочееВремя Цикл
				
				Для ДеньНедели = 1 По 5 Цикл
					ЭлементНастройкиДоступногоВремени = НастройкаДоступногоВремени.Добавить();
					ЭлементНастройкиДоступногоВремени.Сотрудник = Сотрудник;
...
Процедура ДобавитьПромежутокВремениКВремениСтрокой(ВремяСтрокой, ВремяНачала, ВремяОкончания)
...
Процедура УстановитьДоступностьЭлементов()
	...
	ДоступенПоРасписанию = (ДоступностьСотрудника 
		= ПредопределенноеЗначение("Перечисление.ЦБР_ДоступноеВремяСотрудников.ДоступенПоРасписанию"));
...
Процедура ЗаписатьНастройкиДоступногоВремениИзРеквизитовФормы()
	...
	РаботаСРабочимКалендаремСервер.УстановитьНастройкуДоступностиСотрудника(
		Сотрудник,
		ДоступностьСотрудника);

...
Процедура ПередЗаписью(Отказ, Замещение)
	...
	// Для записи набора записей требуем чтобы был установлен отбор по сотруднику
	Сотрудник = Отбор.Найти("Сотрудник").Значение;
	Если Не ЗначениеЗаполнено(Сотрудник) Тогда
		ВызватьИсключение НСтр("ru = 'Запрещено записывать набор регистра сведений ""Доступное время сотрудников"" без установки отбора по сотруднику.'");
	КонецЕсли;
...
Функция ЗаписиСовпадают(Запись1, Запись2)
	...
	Возврат Запись1.ДеньНедели = Запись2.ДеньНедели
		...
		И Запись1.Сотрудник = Запись2.Сотрудник;
	...
	Пока ТребуетсяВнестиКоррективы Цикл
		...
		Для Каждого Запись Из МассивЗаписей Цикл
			...
			Для Каждого ВнесеннаяЗапись Из ЭтотОбъект Цикл
				...
				Если Запись.Сотрудник = ВнесеннаяЗапись.Сотрудник 

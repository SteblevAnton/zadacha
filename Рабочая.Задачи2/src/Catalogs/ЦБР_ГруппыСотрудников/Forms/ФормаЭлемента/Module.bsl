...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Для Каждого Стр Из Объект.Состав Цикл
		Если Не Стр.Разработчик И Не Стр.Аналитик Тогда
			ТекстСообщения = СтрШаблон("Для пользователя %1 не задана роль",Стр.Пользователь);
			Индекс = Стр.НомерСтроки - 1;
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения,Объект.Ссылка,"Объект.Состав[" + Строка(Индекс) + "].Пользователь",,Отказ);
			КонецЕсли;
	КонецЦикла;
